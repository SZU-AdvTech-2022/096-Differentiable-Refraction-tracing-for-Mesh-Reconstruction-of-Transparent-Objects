...
class Loss_calculator:
    ...
    def vh_loss(self):
        ...
        for v in np.arange(0,72,9):
            ...
            index, output = scene.primary_visibility(silhouette_edge, camera_M, origin[0], detach_depth=True) #这里要用到光线与三角形网格相交的信息
    ...
    def ray_loss(self):
        ...
        render_out_ori, render_out_dir, render_mask = scene.render_transparent(origin, ray_dir) #这里得到的是折射两次之后的折射光线的信息
...
def optimize(HyperParams, output=True):
    ...
    def setup_opt(scene, lr, HyperParams):
        ...
        opt = torch.optim.SGD([parameter], lr=lr, momentum = HyperParams['momentum'] , nesterov =True) #优化器设置
    ...
    for i_pass in range(HyperParams['Pass']):
        ...
        init_vertices, parameter, opt = setup_opt(scene, lr, HyperParams) #初始化顶点和参数

        for it in range(HyperParams['Iters']):
            # Zero out gradients before each iteration
            opt.zero_grad()

            vertices = init_vertices + parameter  #对参数求梯度，更新顶点位置，从而更新trimesh中的三角形网格模型
